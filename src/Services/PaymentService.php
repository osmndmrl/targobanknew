...
use TARGOBANK\Services\Database\AccountService;
...
use TARGOBANK\Services\Database\SettingsService;
use TARGOBANK\Services\ContactService;
...
class PaymentService
{
    ...
    /**
     * Get the type of payment from the content of the TARGOBANK container
     *
     * @return string
     */
    ...
    /**
     * Get the TARGOBANK payment content
     *
     * @param Basket $basket
     * @param string $mode
    ...
    public function getPaymentContent(Basket $basket, $mode = PaymentHelper::MODE_PAYPAL, $additionalRequestParams=[]):string
    {
        ...
        // Prepare the TARGOBANK payment
        $preparePaymentResult = $this->libService->libPreparePayment($payPalRequestParams);
        $this->getLogger('TARGOBANK_PaymentService')->debug('preparePayment', $preparePaymentResult);
        ...
        // Store the TARGOBANK Pay ID in the session
        ...
        // Get the content of the TARGOBANK container
    ...
    /**
     * Execute the TARGOBANK payment
     *
     * @return array|string
     */
    public function executePayment($mode = PaymentHelper::MODE_PAYPAL)
    {
        // Load the mandatory TARGOBANK data from session
        ...
        // Set the execute parameters for the TARGOBANK payment
        ...
        $this->getLogger('TARGOBANK_PaymentService')->debug('executePayment', $executeParams);
     ...
     * @param $paymentId
     */
    public function handleTARGOBANKCustomer($paymentId, $mode=PaymentHelper::MODE_PAYPAL)
    {
        $response = $this->getPaymentDetails($paymentId, $mode);

        // update or create a contact
        $this->contactService->handleTARGOBANKContact($response['payer']);
    }
    ...
    public function getPaymentDetails($paymentId, $mode=PaymentHelper::MODE_PAYPAL)
    {
        ...
        $this->getLogger('TARGOBANK_PaymentService')->debug('getPaymentDetails', $response);

        return $response;
    }

    /**
     * request the targobank sale for the given saleId
     *
     * @param $saleId
     * @return array
     */
    public function getSaleDetails($saleId)
    {
        ...
        $this->getLogger('TARGOBANK_PaymentService')->debug('getSaleDetails', $saleDetailsResult);
...
//        $settingsService->setSettingsValue(SettingsService::WEB_PROFILE, $webProfileResult);
    ...
    /**
     * Fill and return the TARGOBANK parameters
     *
     * @param Basket $basket
     * @param String $mode
     * @return array
    ...
    public function getPaypalParams(Basket $basket = null, $mode=PaymentHelper::MODE_PAYPAL)
    {
        ...
        // Set the TARGOBANK Web Profile ID
        $webProfileId = $this->config->get('TARGOBANK.webProfileID');
        ...
        /**
         * Don't send the address to targobank when using targobank plus during the first request
    ...
    {
        $settingType = 'targobank';
        if($mode == PaymentHelper::MODE_PAYPAL_INSTALLMENT)
        {
            $settingType = 'targobank_installment';
        }
     ...
     * @return array|null
     */
    public function loadCurrentSettings($settingsType='targobank')
    {
     ...
     * @return array
     */
    public function loadCurrentAccountSettings($settingsType='targobank')
    {
        $account = [];
        $accountId = 0;
    ...
    /**
     * @return \TARGOBANK\Services\SessionStorageService
     */
    public function getSessionStorage(): \TARGOBANK\Services\SessionStorageService
    {
