...
class TARGOBANKPlusService extends PaymentService
{
    /**
     * @param Basket $basket
     * @return string
    ...
    public function getPaymentWallContent(Basket $basket, Checkout $checkout, CountryRepositoryContract $countryRepositoryContract)
    {
        ...
        $account = $this->loadCurrentAccountSettings('targobank');
        $mode = 'sandbox';
        ...
        if($this->getReturnType() == 'redirectUrl')
        {
            ...
            if(is_array($currentPaymentMethods) && count($currentPaymentMethods) > 0)
            {
                ...
                {
                    if($paymentMethod->id == $this->getPaymentHelper()->getTARGOBANKMopIdByPaymentKey(PaymentHelper::PAYMENTKEY_PAYPALPLUS))
                    {
                        continue;
                    }

                    if($paymentMethod->id == $this->getPaymentHelper()->getTARGOBANKMopIdByPaymentKey(PaymentHelper::PAYMENTKEY_PAYPALINSTALLMENT))
                    {
                        $thirdPartyPaymentMethods[] = [
            ...
            $content = '<script src="https://www.targobankobjects.com/webstatic/ppplus/ppplus.min.js" type="text/javascript"></script>
                                <header class="m-b-1">
                                    <h3>Zahlungsart</h3>
                                </header>
                                <div id="ppplus"> </div>
                ...
                $content .= '           "thirdPartyPaymentMethods" : '.json_encode($thirdPartyPaymentMethods).',
                                        "enableContinue": function(){
                                            switch (ppp.getPaymentMethod())
                                            {
                                                '.implode("\n",$changeCase).'
                                                default:
                                                    $.post("/payment/payPalPlus/changePaymentMethod/", { "paymentMethod" : "'.$this->getPaymentHelper()->getTARGOBANKMopIdByPaymentKey(PaymentHelper::PAYMENTKEY_PAYPALPLUS).'" } );
                                                    document.dispatchEvent(new CustomEvent("afterPaymentMethodChanged", {detail: '.$this->getPaymentHelper()->getTARGOBANKMopIdByPaymentKey(PaymentHelper::PAYMENTKEY_PAYPALPLUS).'}));
                                                    break;
                                            }
                                        },';
            }
    ...
    /**
     * update the targobank payment with the item data and the address
     *
     * @param Basket $basket
     * @return string
     */
