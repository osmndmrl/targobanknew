...
class TARGOBANKInstallmentService extends PaymentService
{
    /**
     * @param Basket $basket
     * @return string
    ...
    public function calculateFinancingCosts(Twig $twig, $amount=0)
    {
        if($amount > 98.99 && $amount < 5000)
        {
            ...
            return $twig->render('TARGOBANK::TARGOBANKInstallment.InstallmentOverlay', ['basketAmount'=>$amount, 'financingOptions'=>$qualifyingFinancingOptions, 'merchantName'=>'Testfirma', 'merchantAddress'=>'Teststraße 1, 34117 Kassel']);
        }
    ...
    {
        $account = $this->loadCurrentAccountSettings('targobank_installment');
    ...
    /**
     * Load the financing costs from the TARGOBANK payment details
     *
     * @param $paymentId
     * @param string $mode
     * @return mixed|null
